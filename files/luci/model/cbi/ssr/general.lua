...
if not has_redir and not has_local and not has_tunnel then
	return Map(ssr, "%s - %s" %{translate("ssr"),
		translate("General Settings")}, '<b style="color:red">ssr-libev binary file not found.</b>')
end
...
m = Map(ssr, "%s - %s" %{translate("ssr"), translate("General Settings")})
m.template = "ssr/general"
